"""
app/utils/tools/travel_tools.py
---------------------------------
Flight, hotel and tourist-place search tools powered by SerpAPI.
"""
...
INDIA_AIRPORTS: dict = {
    ...
    "cochin": "COK", "goa": "GOI", "panaji": "GOI", "lucknow": "LKO",
    "bhopal": "BHO", "indore": "IDR", "nagpur": "NAG", "surat": "STV",
    "vadodara": "BDQ", "amritsar": "ATQ", "chandigarh": "IXC", "patna": "PAT",
    "ranchi": "IXR", "bhubaneswar": "BBI", "visakhapatnam": "VTZ", "vizag": "VTZ",
    "coimbatore": "CJB", "madurai": "IXM", "tiruchirappalli": "TRZ", "trichy": "TRZ",
    "mangalore": "IXE", "mangaluru": "IXE", "thiruvananthapuram": "TRV",
    "trivandrum": "TRV", "kozhikode": "CCJ", "calicut": "CCJ", "guwahati": "GAU",
    "imphal": "IMF", "agartala": "IXA", "dibrugarh": "DIB", "jorhat": "JRH",
    "silchar": "IXS", "varanasi": "VNS", "dehradun": "DED", "jammu": "IXJ",
    "srinagar": "SXR", "leh": "IXL", "udaipur": "UDR", "jodhpur": "JDH",
    "aurangabad": "IXU", "raipur": "RPR", "hubli": "HBX", "belgaum": "IXG",
    "port blair": "IXZ",
}
...
def search_flights(origin: str, destination: str, date: str, adults: int = 1, currency: str = "INR") -> str:
    """
    Search Google Flights for available flights from origin to destination.
    origin and destination accept Indian city names or IATA codes.
    ...
    try:
        ...
        logger.info({"event": "search_flights", "origin": origin, "departure_id": departure_id,
                     "dest": destination, "arrival_id": arrival_id, "date": outbound_date})
        data = _serp(params)
...
def search_hotels(destination: str, currency: str = "INR") -> str:
    ...
    Check-in is tomorrow and check-out is the day after automatically.
    """
    if not API_KEY:
        return "Error: SERPAPI_KEY is not configured. Add it to your .env file."
    ...
    try:
        ...
        logger.info({"event": "search_hotels", "destination": destination,
                     "check_in": check_in_date, "check_out": check_out_date})
        data = _serp(params)
        properties = data.get("properties", [])
        if not properties:
            return f"No hotels found in '{destination}'. Try a broader or different city name."
...
def search_tourist_places(destination: str) -> str:
    """
    Search for top tourist attractions in a destination city.
    Returns top attractions with address, rating and website.
    """
    if not API_KEY:
        return "Error: SERPAPI_KEY is not configured. Add it to your .env file."
    try:
        query = f"tourist attractions in {destination.strip()}"
        params = {"engine": "google_maps", "q": query, "type": "search", "hl": "en", "gl": "in"}
        logger.info({"event": "search_tourist_places", "destination": destination})
        data = _serp(params)
        results = data.get("local_results", [])
        if not results:
            fallback_params = {"engine": "google", "q": f"top tourist places to visit in {destination}", "num": 8}
