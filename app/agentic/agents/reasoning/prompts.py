"""
app/agentic/agents/reasoning/prompts.py
-----------------------------------------
Prompt templates for reasoning and data-collector agents.
Replaces app/engine/agents/prompt_generator.py.
"""

from app.utils.tools.registry import tool_metadata
...
def generate_prompt(agent_def) -> str:
    """Return the appropriate system prompt for an agent definition."""
    behavior = getattr(agent_def, "behavior", "task_executor")
    if behavior == "data_collector":
        return _prompt_data_collector(agent_def)
