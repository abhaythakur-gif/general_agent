"""
app/utils/tools/registry.py
-----------------------------
Tool registry: maps tool names → callable functions.
All tool metadata is also stored here and seeded into MongoDB on startup.
"""

from app.utils.tools.travel_tools import (
    search_flights, search_hotels, search_tourist_places,
)
from app.utils.tools.weather_tools import (
    get_current_weather, get_weather_forecast, get_air_quality,
    get_astronomy, get_historical_weather, search_timezone,
)
from app.utils.tools.search_tools import (
    web_search, news_search, image_search, places_search, shopping_search,
    youtube_search, jobs_search, scholar_search, stock_search, autocomplete_search,
)
...
def get_tools_for_agent(tool_names: list) -> dict:
    """Return a {name: callable} dict for the given tool name list."""
    return {name: tool_registry[name] for name in tool_names if name in tool_registry}


def seed_tools_to_db() -> None:
    """Upsert all tool metadata into MongoDB. Safe to call on every startup."""
    from app.utils.common.storage import seed_tools
    seed_tools(list(tool_metadata.values()))


def list_tools() -> list:
    """Return tool metadata from MongoDB, falling back to the hardcoded dict."""
    try:
        from app.utils.common.storage import list_tools as _db_list
        result = _db_list()
        if result:
            return result
    except Exception:
