...
def _evaluate_condition(expression: str, state: dict, agent_name: str) -> bool:
    """
    Safely evaluate a boolean condition expression against the current
    workflow state.  Raises RuntimeError on bad syntax or missing variable.
    """
    try:
        result = eval(expression, {"__builtins__": {}}, state)
        return bool(result)
...
def _build_parallel_map(parallel_groups: List[List[str]]) -> dict:
    """Return {agent_id: frozenset(group)} for every agent in a parallel group."""
    m = {}
...
def _execute_from(
    ...
    """
    Inner loop — runs agents from start_idx onwards.
    Returns a WorkflowResult dict.
    """
    ...
    parallel_done = set()   # frozensets of already-executed parallel groups
    ...
    for idx in range(start_idx, total):
        ...
        if group_key is not None:
            ...
            log("parallel_group_start", step=idx + 1,
            ...
            with ThreadPoolExecutor(max_workers=len(group_agents)) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        log("parallel_agent_done", agent=ag.name,
                            produced=list(updates.keys()))
        ...
        # ── Condition gate ────────────────────────────────────────────────────
        run_if = getattr(agent_def, "run_if", None)
        if run_if:
        ...
        if behavior == "data_collector":
            ...
            if collection_status == "incomplete":
                ...
                storage.update_execution(execution_id, {
                    "status":             "paused",
                    "current_step":       idx,       # resume will re-run this same agent
...
def start_workflow(
    ...
    Returns a WorkflowResult dict with status "completed" or "paused".
    """
    ...
    log("workflow_start",
        ...
        initial_inputs=list(initial_inputs.keys()),
        parallel_groups=parallel_groups)

    return _execute_from(
        agent_defs, state, 0, parallel_groups,
...
def resume_workflow(
    ...
    """
    Resume a paused workflow after receiving new user input.

    The paused data_collector agent is re-run with the new user message
    merged into state. If it becomes complete, the rest of the pipeline runs.
    """
    collected_logs: List[dict] = []
    ...
    # Merge previous logs so the UI sees the full history
    collected_logs.extend(prev_logs)

    # Update user_message in state with new input
...
def run_workflow(
    ...
    """
    Legacy entry point kept for compatibility with existing code.
    Returns the WorkflowResult dict (not just state).

    NOTE: The new UI uses start_workflow/resume_workflow directly.
    """
